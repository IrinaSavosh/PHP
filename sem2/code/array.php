 <!-- В памяти значения массивов записываются хаотично, каждый элемент ссылается на другой. Это замедляет процесс поиска, но позволяет проще добавлять или удалять элементы-->

<?php

$arr = [
  'Московская' => [
      'Москва',
      'Москва',
      'Москва',
      'Москва',
  ]
];
$str="Привет мир!";

//$n % 10;
/*
0 часов	минут	10 часов минут 20 часов минут	30 минут  40 минут
1 час	минута	11 часов минут 21 час	минута	31 минута 41 минута
2 часа	минуты	12 часов минут 22 часа	минуты	32 минуты 42 минуты
3 часа	минуты	13 часов минут 23 часа	минуты	33 минуты 43 минуты
4 часа	минуты	14 часов минут 24 часа	минуты	34 минуты 44 минуты
5 часов	минут	15 часов минут 25		минут	35 минут  45 минут
6 часов	минут	16 часов минут 26		минут	36 минут  46 минут
7 часов минут	17 часов минут 27		минут	37 минут  47 минут
8 часов	минут	18 часов минут 28		минут	38 минут  48 минут
9 часов минут	19 часов минут 29		минут	39 минут  49 минут
*/

$alfabet = [
    'а' => 'a',   'б' => 'b',   'в' => 'v',
    'г' => 'g',   'д' => 'd',   'е' => 'e',
    'ё' => 'e',   'ж' => 'zh',  'з' => 'z',
    'и' => 'i',   'й' => 'y',   'к' => 'k',
    'л' => 'l',   'м' => 'm',   'н' => 'n',
    'о' => 'o',   'п' => 'p',   'р' => 'r',
    'с' => 's',   'т' => 't',   'у' => 'u',
    'ф' => 'f',   'х' => 'h',   'ц' => 'c',
    'ч' => 'ch',  'ш' => 'sh',  'щ' => 'sch',
    'ь' => '\'',  'ы' => 'y',   'ъ' => '\'',
    'э' => 'e',   'ю' => 'yu',  'я' => 'ya'
];

$users = [
    [
        'name' => 'Alex',
        'age' => 24
    ],
    [
        'name' => 'Bob',
        'age' => 77
    ]
];

foreach ($users as $user) {
    print_r($user['name']);
}


// Работа с массивами ИНТЕРЕСНАЯ
$a = [1,2,3];
echo $a[45]; //выведет ошибку, не отработает, но не завершится аварийно
echo $a[1];

$a[] = 5; // добавление значения по последнему индексу + 1
$a[45] = 6; // добавление значения по индексу 45
$a[] = 7; //добавление значения по индексу следующего за последним, за сорок пятым

print_r($a); // функция, которая печатает содержание массива

$a[2] = null; //не удаляет элемент, а делает его пустотой
unset($a[45]); //функция удаляет полностью элемент
print_r($a); 

